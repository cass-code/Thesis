ac_kotze  <- function(series,max.lag=18,main=NULL){
  num=length(series)
  ...
  barplot(ACF, ylab='ACF', xlab='LAG', ylim=c(minu,1), col='#000050', border=NA, names.arg=LAG)
  box()
  abline(h=c(0,L,U), lty=c(1,3,3), lwd = 2.5, col="darkgrey")
  barplot(PACF, ylab='PACF', xlab='LAG', ylim=c(minu,1), col='#000050', border=NA, names.arg=LAG)
  box()
  abline(h=c(0,L,U), lty=c(1,3,3), lwd = 2.5, col="darkgrey")
